...
class SingerManager extends React.Component{
    ...
    componentDidMount(){
        fetch('/get_singer',{credentials:'include'})
            .then(res=>res.json())
            .then(r=>{
                if(r.code===4){
    ...
    render(){
        const tableData={
            columns:[
                ...
                {title:'歌手名称',key:'sname',render:(record)=>(
                    <div className="cateUpdateCell">
                        {record.sname}
                        &nbsp;&nbsp;&nbsp;
                        <Popconfirm
                            title={(<Input onChange={(e)=>this.change(e)} defaultValue={record.sname}/>)}
                            onConfirm={()=>this.update(record.sid,'sname',record)}
                        >
                            <Button shape="circle" type="dashed"><Icon type="edit" /></Button>
                        </Popconfirm>
                    </div>
                )},
                {title:'歌手图片',key:'spic',render:(record)=>{
                    let o = [];
                    if(record.spic){
                        o.push({
                            uid:-1,
                    ...
                    return (
                        <div>
                            ...
                            <Modal visible={record.show} footer={null} onCancel={()=>this.hideModel(record.sid)}>
                                <img style={{ width: '100%' }} src={record.spic} alt=""/>
                            </Modal>
                        </div>
                    )
                }
